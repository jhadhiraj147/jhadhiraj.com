      ...
      <div className="mx-auto max-w-4xl">
        <AnimatedSection>
          <SectionHeading
            ...
            title="More Projects"
            subtitle="Distributed and concurrent systems."
          />
        </AnimatedSection>
          ...
          {projects.map((proj, i) => (
            <AnimatedSection key={proj.title} delay={i * 0.1}>
              <motion.article
                ...
                className="flex h-full flex-col rounded-2xl border border-slate-200 bg-white/70 p-7 shadow-sm transition-shadow duration-300 hover:shadow-lg"
              >
                <div className="mb-5 flex items-baseline justify-between">
                  <span className="font-display text-sm text-slate-300">
                    0{i + 1}
                  </span>
                  <span className="inline-flex items-center gap-1.5 font-body text-xs text-slate-400">
